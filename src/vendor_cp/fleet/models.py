...
class Deployment(Base, TimestampMixin):
    ...
    # Planning owns the pointed-to row, while Fleet owns this aggregate pointer.
    # Clearing it invalidates approval without deleting any immutable evidence.
    current_plan_id: Mapped[UUID | None] = mapped_column(
        sa.Uuid(), ForeignKey("deployment_plans.id", ondelete="RESTRICT"), nullable=True
    )
    latest_plan_revision: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0
    )


class DeploymentCapabilityInstance(Base, TimestampMixin):
    """Stable identity of one reusable capability inside a deployment.

    Capability contracts may advance across desired-state revisions.  The
    instance reference does not: it is the deployment-local node to which one
    Integrator binding, desired operation document and plan command attach.
    """

    __tablename__ = "deployment_capability_instances"
    __table_args__ = (
        UniqueConstraint(
            "deployment_id",
...
class DeploymentDesiredStateVersion(Base, TimestampMixin):
    ...
    deployment identity, selected components/capabilities/operations/checks and
    ...
    selected_operations: Mapped[list[dict[str, Any]]] = mapped_column(
        _JSON, nullable=False
    )
    ...
    desired_operation_inputs: Mapped[dict[str, Any]] = mapped_column(
        _JSON, nullable=False
    )
    selected_composition_edges: Mapped[list[dict[str, Any]]] = mapped_column(
        _JSON, nullable=False
    )
...
@event.listens_for(DeploymentCapabilityInstance, "before_update")
def _refuse_capability_instance_update(
    _mapper: Mapper[DeploymentCapabilityInstance],
    _connection: sa.Connection,
    target: DeploymentCapabilityInstance,
) -> None:
    raise ConflictError(
        f"deployment capability instance {target.deployment_id}/"
        f"{target.capability_instance_ref} is immutable"
    )


@event.listens_for(DeploymentCapabilityInstance, "before_delete")
def _refuse_capability_instance_delete(
    _mapper: Mapper[DeploymentCapabilityInstance],
    _connection: sa.Connection,
    target: DeploymentCapabilityInstance,
) -> None:
...
# Register the target of Deployment.current_plan_id even when a focused Fleet
# unit test builds metadata without importing the composed assembly first.
import vendor_cp.planning.models  # noqa: E402, F401
