...
- **State:** remediation facility implemented; live rotation outstanding.
...
- **Refused recurrence:** identity is read only from `.Config.Image` and the OCI
  revision label. The rotation adapter contains no Docker environment-inspect
  operation, and tests assert the forbidden selector is absent.
