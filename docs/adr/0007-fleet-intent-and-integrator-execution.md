...
- **Managed profile** — immutable/versioned required and allowed-optional
  component policy, capability requirements, dependency graph, configuration
  field schema and verification checks. It contains no customer configuration
  and does not make a deployment's optional selection.
...
- **Desired-state snapshot** — content-addressed selected components, typed
  configuration references, required provider-neutral capabilities, update
  authority and verification contract.
...
not manufacture a capability or entitlement that a product release manifest and
allocation do not declare.

### 4. Configuration is typed; artifact evidence is a plan input

Ordinary public configuration is constrained by declared field type (for
...
Vendor never accepts or stores a raw secret value.

Stack 1 does not pretend a managed profile is an artifact catalogue. ADR-0008's
immutable bundle manifest records exact artifacts and their source class:
...
- a pinned third-party image/package is evidenced by exact upstream artifact
  digest plus a Dotmac-authored compatibility/verification profile;
