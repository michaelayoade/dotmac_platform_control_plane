...
from vendor_cp.fleet.models import DeploymentDesiredStateVersion
...
def _values(*, workspace: bool = False) -> tuple[service.ConfigurationValue, ...]:
    ...
    return tuple(
        service.ConfigurationValue(field_code=code, value=value)
...
def _collaboration_values() -> tuple[service.ConfigurationValue, ...]:
    ...
    return tuple(value for value in _values() if value.field_code in selected)
...
def test_contract_backed_intent_freezes_exact_selected_snapshot(
    ...
    assert {item["capability_code"] for item in desired.selected_capabilities} >= {
        "collaboration.application.lifecycle",
        "identity.realm.lifecycle",
    }
    assert {item["endpoint_code"] for item in desired.selected_endpoints} >= {
        "collaboration.application.ensure",
        "identity.realm.ensure",
    }
    assert desired.configuration_snapshot_ref == "config:nhia/collaboration@v1"
    assert desired.configuration_schema_version == 1
    assert desired.configuration_snapshot["identity_admin_secret_ref"] == (
...
def test_suite_optional_workspace_is_per_deployment_not_per_profile(
    ...
    assert "workspace_endpoint" in with_row.configuration_snapshot
