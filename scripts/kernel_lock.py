...
#: because a new version is entitled to require different things -- and it
...
#: checked by `metadata_binding_problems`.
...
def wheel_requires_dist(wheel: Path) -> list[str]:
    """The `Requires-Dist` lines of a wheel's own METADATA.

    Read out of the ARTIFACT, not out of a source tree and not out of a
    changelog: the declaration that binds a resolver is the one inside the
    bytes that get installed. The caller is expected to have already proven
    this file's digest against the lock.
    """
    ...
    with zipfile.ZipFile(wheel) as archive:
        ...
        if len(names) != 1:
            raise Refusal(
                f"{wheel.name} carries {len(names)} top-level dist-info METADATA "
    ...
    for line in text.splitlines():
        if not line.strip():
            break  # headers end at the first blank line; the body is prose
        if line.lower().startswith("requires-dist:"):
            requirements.append(line.split(":", 1)[1].strip())
    return requirements


def metadata_dependencies(requires_dist: Iterable[str]) -> dict[str, str]:
    """`Requires-Dist` as the mapping a Poetry lock records, or a refusal.

    Only UNCONDITIONAL requirements belong in a lock entry's `dependencies`;
    a requirement carrying an environment marker or an extra is recorded
    elsewhere. Such a requirement is REFUSED rather than skipped: silently
    ignoring part of the metadata is how a comparison stops being able to
    disagree, and this gate exists to disagree.
    """
    ...
    for requirement in requires_dist:
        ...
        if key in mapping:
            raise Refusal(f"the wheel declares {name} twice in `Requires-Dist`")
        mapping[key] = re.sub(r"\s+", "", specifier)
    return mapping


def _locked_dependencies(entry: dict[str, Any]) -> dict[str, str]:
    """A lock entry's `dependencies`, keyed and spelled for comparison."""
    ...
    for name, constraint in declared.items():
        ...
        mapping[_normalised(str(name))] = re.sub(r"\s+", "", str(constraint))
    return mapping
...
def metadata_binding_problems(
    ...
    entitled to require different things -- that is exactly why Control a13
    forces the kernel pin to move. But it may not change them FREELY: the
    resolved values must equal what the wheel itself declares. Otherwise the
    one field a movement is allowed to alter becomes the field through which an
    arbitrary constraint could arrive inside a reviewed pin change.

    The wheel is bound to the lock by digest FIRST. Reading metadata out of a
    file whose hash nobody checked would be reading an unverified claim.
    """
    ...
    if wheel.name not in locked_files:
        return [
            f"{wheel.name} is not named by the lock entry for "
            f"{entry.get('name')} {entry.get('version')}, so its metadata "
            "describes bytes this lock does not claim"
        ]
    acquired = digests.get(wheel.name)
    if acquired is None:
        return [f"{wheel.name} has no acquired digest to verify its metadata against"]
    if locked_files[wheel.name] != f"sha256:{acquired}":
        return [
            f"{wheel.name}: the lock says {locked_files[wheel.name]!r} but the "
            f"acquired bytes hash to {'sha256:' + acquired!r}; refusing to read "
            "metadata out of bytes that do not match the lock"
        ]

    expected = metadata_dependencies(wheel_requires_dist(wheel))
    actual = _locked_dependencies(entry)
    for name in sorted(set(expected) | set(actual)):
        if name not in actual:
            problems.append(
                f"the wheel requires {name} {expected[name]!r}, which the lock "
                "entry does not record"
            )
        elif name not in expected:
            problems.append(
                f"the lock records a dependency on {name} {actual[name]!r} that "
                "the wheel's `Requires-Dist` does not declare"
            )
        elif expected[name] != actual[name]:
            problems.append(
                f"{name}: the wheel declares {expected[name]!r}, the lock "
                f"records {actual[name]!r}"
            )
    return problems
...
def _declared_delta_problems(
    ...
    # field that must be bound to something outside the lock. Not optional: a
    # declared movement with no bundle to check against would permit exactly
    # the change this binding exists to constrain.
    if bundle is None:
        problems.append(
